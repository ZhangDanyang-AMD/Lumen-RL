...
class RayPPOTrainer:
    ...
    def fit(self):
        ...
        # Dynamic sampling (filter_groups) accumulators
        _fg_cfg = getattr(self.config.algorithm, "filter_groups", None)
        _fg_enabled = _fg_cfg is not None and getattr(_fg_cfg, "enable", False)
        if not _fg_enabled and os.environ.get("VERL_FILTER_GROUPS_ENABLE", "0") == "1":
            _fg_enabled = True

            class _FGConfig:
                enable = True
                metric = os.environ.get("VERL_FILTER_GROUPS_METRIC", "acc")
                max_num_gen_batches = int(os.environ.get("VERL_FILTER_GROUPS_MAX_GEN", "10"))

            _fg_cfg = _FGConfig()
            print(f"[filter_groups] Enabled via env: metric={_fg_cfg.metric}, max_gen={_fg_cfg.max_num_gen_batches}")
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # ── Dynamic sampling: filter uninformative groups ─────────
                    if _fg_enabled:
                        _fg_num_gen_batches += 1
                        metric_name = _fg_cfg.metric
                        ...
                        for uid, mval in zip(
                            ...
                            prompt_uid2metric_vals[uid].append(float(mval))
                        ...
                        _fg_num_prompts += len(kept_prompt_uids)

                        kept_idxs = [
                            i for i, uid in enumerate(batch.non_tensor_batch["uid"])
                            if uid in kept_prompt_uids
                        ]
                        filtered_batch = batch[kept_idxs] if kept_idxs else None

                        if filtered_batch is not None:
                            _fg_accumulated_batch = (
                                filtered_batch
                                if _fg_accumulated_batch is None
                                else DataProto.concat([_fg_accumulated_batch, filtered_batch])
                            )

                        prompt_bsz = self.config.data.train_batch_size
                        if _fg_num_prompts < prompt_bsz:
                            max_gen = _fg_cfg.max_num_gen_batches
                            if max_gen <= 0 or _fg_num_gen_batches < max_gen:
                                print(
                                    f"[filter_groups] {_fg_num_prompts=} < {prompt_bsz=}, "
                                    f"{_fg_num_gen_batches=}. Keep generating..."
                                )
                                continue
                            else:
                                print(
                                    f"[filter_groups] {_fg_num_gen_batches=} >= {max_gen=}. "
                                    f"Only {_fg_num_prompts} valid prompts. Using what we have."
                                )

                        if _fg_accumulated_batch is not None:
                        ...
                        metrics["train/num_valid_prompts"] = _fg_num_prompts
                        metrics["train/filter_ratio"] = 1.0 - (
                            _fg_num_prompts / (
                                _fg_num_gen_batches * len(prompt_uid2metric_vals)
                            )
                        ) if _fg_num_gen_batches > 0 and len(prompt_uid2metric_vals) > 0 else 0.0
