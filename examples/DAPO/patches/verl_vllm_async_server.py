...
class vLLMHttpServer:
    ...
    async def collective_rpc(
        ...
        import logging as _log; _dbg = _log.getLogger("verl.debug.server_crpc"); _dbg.setLevel(_log.INFO)
        _dbg.info(f"SERVER_CRPC: received collective_rpc({method}), forwarding to engine...")
        ...
        _dbg.info(f"SERVER_CRPC: collective_rpc({method}) completed")
        return result
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None, dp_rpc_port: int = None):
        ...
        import torch
        _enable_sleep = self.config.enable_sleep_mode
        _is_rocm = bool(getattr(torch.version, 'hip', None))
        logger.info(f"enable_sleep_mode: {_enable_sleep} (config={self.config.enable_sleep_mode}, rocm={_is_rocm})")
        if not _enable_sleep or _is_rocm:
        ...
        args = {
            ...
            "distributed_executor_backend": "uni" if self.config.tensor_model_parallel_size == 1 else "mp",
    ...
    async def run_server(self, args: argparse.Namespace):
        ...
        logger.info("DEBUG: creating AsyncLLM engine...")
        engine_client = AsyncLLM.from_vllm_config(vllm_config=vllm_config, usage_context=usage_context, **kwargs)
        logger.info("DEBUG: AsyncLLM created, calling reset_mm_cache...")

        # Don't keep the dummy data in memory
        await engine_client.reset_mm_cache()
        logger.info("DEBUG: reset_mm_cache done, calling monkey_patch_model...")
        await engine_client.collective_rpc(
            method="monkey_patch_model", kwargs={"vocab_size": len(self.model_config.tokenizer)}
        )
        logger.info("DEBUG: monkey_patch_model done")
        ...
        logger.info("DEBUG: starting uvicorn HTTP server...")
        self._server_port, self._server_task = await run_uvicorn(app, args, self._server_address)
        logger.info(f"DEBUG: uvicorn started on port {self._server_port}")
    ...
    async def sleep(self):
        ...
        _dbg = _log.getLogger("verl.debug.server_sleep"); _dbg.setLevel(_log.INFO)
        _dbg.info("SERVER_SLEEP: entered, node_rank=%s, free_cache_engine=%s, rollout_mode=%s",
                   self.node_rank, self.config.free_cache_engine, self.rollout_mode)
        print(f"[SERVER_SLEEP] entered, node_rank={self.node_rank}, free_cache_engine={self.config.free_cache_engine}, mode={self.rollout_mode}", flush=True)
        if self.node_rank != 0 or not self.config.free_cache_engine:
            _dbg.info("SERVER_SLEEP: skipping (node_rank=%s, free_cache=%s)", self.node_rank, self.config.free_cache_engine)
            print(f"[SERVER_SLEEP] skipping", flush=True)
            return

        if self.rollout_mode == RolloutMode.HYBRID:
            _dbg.info("SERVER_SLEEP: calling _sleep_hybrid()")
            print("[SERVER_SLEEP] calling _sleep_hybrid()", flush=True)
            await self._sleep_hybrid()
            _dbg.info("SERVER_SLEEP: _sleep_hybrid() completed")
            print("[SERVER_SLEEP] _sleep_hybrid() completed", flush=True)
        elif self.rollout_mode == RolloutMode.COLOCATED:
            _dbg.info("SERVER_SLEEP: colocated, calling engine.sleep(level=1)")
