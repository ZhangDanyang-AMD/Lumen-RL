...
class ServerAdapter(BaseRollout):
    ...
    async def _execute_method(
        ...
        """
        import logging as _log; _dbg = _log.getLogger("verl.debug.execute_method"); _dbg.setLevel(_log.INFO)
        if self.rollout_rank != 0:
            _dbg.info(f"EXECUTE_METHOD: skipping {method} (rollout_rank={self.rollout_rank}!=0)")
            return None

        # Lazy init http server adapter because http server is launched after hybrid engine.
        if self.server_handle is None:
            _dbg.info(f"EXECUTE_METHOD: lazy init server_handle for vllm_server_{self.replica_rank}_{self.node_rank}")
            self.server_handle = ray.get_actor(f"vllm_server_{self.replica_rank}_{self.node_rank}")
            _dbg.info(f"EXECUTE_METHOD: got server_handle={self.server_handle}")

        _dbg.info(f"EXECUTE_METHOD: calling collective_rpc.remote({method}, non_block={non_block})")
        future = self.server_handle.collective_rpc.remote(method, timeout=timeout, args=args, kwargs=kwargs)
        _dbg.info(f"EXECUTE_METHOD: collective_rpc.remote dispatched, future={future}")
    ...
    async def release(self):
        ...
        import logging as _log; _dbg = _log.getLogger("verl.debug.release"); _dbg.setLevel(_log.INFO)
        _dbg.info("RELEASE: entered, free_cache_engine=%s, sleep_level=%s, rollout_rank=%s",
                   self.config.free_cache_engine, getattr(self, "sleep_level", "MISSING"),
                   getattr(self, "rollout_rank", "MISSING"))
        if self.config.free_cache_engine:
            _dbg.info("RELEASE: calling sleep(level=%s)...", self.sleep_level)
            await self._execute_method("sleep", kwargs={"level": self.sleep_level})
            _dbg.info("RELEASE: sleep() completed")
        else:
            _dbg.info("RELEASE: free_cache_engine=False, skipping")
    ...
    async def update_weights(
        ...
        import logging as _log; _dbg = _log.getLogger("verl.debug.vllm_rollout"); _dbg.setLevel(_log.INFO)
        start_time = time.time()

        _dbg.info(f"UPDATE_WEIGHTS: entered, rollout_rank={self.rollout_rank}, replica_rank={self.replica_rank}, zmq_handle={self.zmq_handle}, use_shm={self.use_shm}")
        future = await self._execute_method(
            "update_weights_from_ipc",
            non_block=True,
            kwargs={**kwargs, "use_shm": self.use_shm},
        )
        _dbg.info(f"UPDATE_WEIGHTS: _execute_method returned, future={future}")
        ...
        _dbg.info(f"UPDATE_WEIGHTS: calling async_send_weights with bucket_size_mb={bucket_size_mb}")
        await sender.async_send_weights(weights)
        _dbg.info("UPDATE_WEIGHTS: async_send_weights done")

        if future is not None:
            _dbg.info("UPDATE_WEIGHTS: awaiting future...")
            await future
            _dbg.info("UPDATE_WEIGHTS: future resolved")
