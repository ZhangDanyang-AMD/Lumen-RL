...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def _build_model_optimizer(
        ...
        if fsdp_strategy == "fsdp":
            ...
            # Lumen FP8ParamManager: apply before FSDP2 wrapping
            import os as _os
            _fp8_env = _os.environ.get("FP8_PARAM_MANAGER", "0")
            print(f"[LUMEN_DEBUG] role={role}, FP8_PARAM_MANAGER={_fp8_env}, pid={_os.getpid()}", flush=True)
            if _fp8_env == "1" and role == "actor":
                try:
                    from lumen.rl.verl.config import VerlLumenArgs
                    from lumen.config import LumenConfig
                    _lumen_args = VerlLumenArgs(
                        model_name_or_path=_os.environ.get("MODEL_NAME", ""),
                        fp8_param_manager=True,
                        use_8bit_adam=False,
                    )
                    _cfg = LumenConfig.from_args(_lumen_args)
                    _cfg.enable(actor_module)
                    print(f"[LUMEN_DEBUG] FP8PM applied successfully to actor, pid={_os.getpid()}", flush=True)
                    logger.info("Lumen FP8PM applied to actor before FSDP2")
                except Exception as _e:
                    import traceback
                    print(f"[LUMEN_DEBUG] FP8PM FAILED: {_e}", flush=True)
                    traceback.print_exc()
                    logger.warning("Lumen FP8PM failed: %s", _e)
    ...
    async def trainer_mode(self):
        """Switch from rollout to training: release vLLM weights + KV cache."""
        import logging as _log, sys
        _dbg = _log.getLogger("verl.debug.trainer_mode"); _dbg.setLevel(_log.INFO)
        _dbg.info("TRAINER_MODE: entered, free_cache_engine=%s", self.config.rollout.free_cache_engine)
        print(f"[TRAINER_MODE] entered, free_cache_engine={self.config.rollout.free_cache_engine}", flush=True)
        if self.config.rollout.free_cache_engine:
            _dbg.info("TRAINER_MODE: calling self.rollout.release()...")
            print("[TRAINER_MODE] calling self.rollout.release()...", flush=True)
            try:
                await self.rollout.release()
                _dbg.info("TRAINER_MODE: release() completed successfully")
                print("[TRAINER_MODE] release() completed successfully", flush=True)
            except Exception as e:
                _dbg.error("TRAINER_MODE: release() FAILED: %s", e, exc_info=True)
                print(f"[TRAINER_MODE] release() FAILED: {e}", flush=True)
        else:
            _dbg.info("TRAINER_MODE: free_cache_engine is False, skipping release")
            print("[TRAINER_MODE] free_cache_engine is False, skipping", flush=True)

    async def rollout_mode(self):
        """Context switch hybridengine to rollout mode."""
        import logging as _log; _dbg = _log.getLogger("verl.debug.rollout_mode"); _dbg.setLevel(_log.INFO)
        _dbg.info("ROLLOUT_MODE: entered")
        ...
        _dbg.info("ROLLOUT_MODE: calling self.rollout.update_weights...")
        await self.rollout.update_weights(per_tensor_param, peft_config=peft_config, base_sync_done=self.base_sync_done)
        _dbg.info("ROLLOUT_MODE: update_weights returned")
