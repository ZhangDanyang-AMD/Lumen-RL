#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
# Kimi K2.5 Eagle3 Draft Model Distillation (SGLang + ATOM + NVFP4)
#
# Off-policy speculative decoding draft model training using teacher hidden
# states. SGLang Engine with ATOM plugin provides NVFP4-quantized inference;
# Mooncake RDMA transfers hidden states to training GPUs.
#
# GPU split strategy (8x GPU):
#   GPUs 0-3: torchrun FSDP2 draft model training (BF16)
#   GPUs 4-7: SGLang + ATOM teacher inference (TP=4, NVFP4)
#
# Data flow:
...
EXP_NAME="kimi-k25-eagle3-sglang-nvfp4"
...
# Model / data paths
MODEL_PATH="${MODEL_PATH:-nvidia/Kimi-K2.5-NVFP4}"
CKPT_DIR="${CKPT_DIR:-results/kimi_k25_eagle3_sglang_nvfp4}"
...
if [ "${SMOKE_TEST}" = true ]; then
    ...
    CONFIG="${SCRIPT_DIR}/configs/eagle3_sglang_nvfp4.yaml"
fi
...
echo "  Teacher:     ${MODEL_PATH} (SGLang+ATOM, GPUs 4-7, TP=4, NVFP4)"
