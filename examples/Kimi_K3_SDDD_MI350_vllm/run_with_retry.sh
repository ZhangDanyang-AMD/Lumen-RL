#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
# Auto-restart wrapper for Kimi K3 DSpark training — MI350.
#
# Absorbs ROCm failure modes by polling log mtime; if no new line for
# HANG_IDLE_SEC (default 900s — longer than K3's TP=8 startup), kills the
# container and lets the wrapper loop restart from the latest checkpoint.
#
...
#   MAX_ATTEMPTS=50 HANG_IDLE_SEC=1200 HF_TOKEN=hf_xxx \
#       bash examples/Kimi_K3_SDDD_MI350_vllm/run_with_retry.sh
# ═══════════════════════════════════════════════════════════════════════════════
set -uo pipefail
...
HANG_IDLE_SEC="${HANG_IDLE_SEC:-900}"
WATCHDOG_POLL_SEC="${WATCHDOG_POLL_SEC:-30}"
CONTAINER_NAME="${CONTAINER_NAME:-kimi_k3_dspark_vllm_mi350}"

LOG_DIR="${REPO_ROOT}/output/Kimi_K3_SDDD/LumenRL"
LOG_FILE="${LOG_DIR}/kimi-k3-dspark-vllm-mi350.log"
...
while [ "${attempt}" -lt "${MAX_ATTEMPTS}" ]; do
    ...
    if [ -f "${LOG_FILE}" ]; then
        rotated="${LOG_DIR}/kimi-k3-dspark-vllm-mi350.attempt-$(printf '%02d' "${attempt}").log"
        cp "${LOG_FILE}" "${rotated}" 2>/dev/null || true
    fi

    # Reap GPU coredumps
    if ls "${REPO_ROOT}"/gpucore.*.gpu >/dev/null 2>&1; then
        docker run --rm -v "${REPO_ROOT}":/host "${DOCKER_IMAGE:-kimi_k3_dspark_vllm_train:latest}" \
